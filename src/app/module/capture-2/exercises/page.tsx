...
const exercises = Array.from({ length: 10 }, (_, idx) => ({
  ...
  sgfPath: `/sgf/01_01_04_${String(idx + 1).padStart(2, '0')}.sgf`,
}));
...
export default function Cattura2ExercisesPage() {
  ...
  useEffect(() => {
    ...
    fetch(currentExercise.sgfPath)
      ...
      .then((text) => {
        ...
        setPrompt(
          root.comment?.trim() ||
            'Cattura le pietre marcate con la mossa corretta.',
        );
      ...
      .catch((err) => {
        if (!active) return;
        console.error(
          `[Cattura 2] Impossibile caricare ${currentExercise.sgfPath}`,
          err,
        );
  ...
  const statusMessage = useMemo(() => {
    ...
    if (status === 'done') return 'Corretto! La cattura è completata.';
    if (status === 'progress')
      return 'Continua la sequenza per completare la cattura.';
    if (status === 'error') return 'Non è la mossa giusta. Riprova.';
    return 'Seleziona la mossa che cattura il gruppo.';
        ...
        <section className="rounded-3xl border border-emerald-100 bg-white/90 p-6 shadow-lg shadow-emerald-100 backdrop-blur">
          <div className="mb-6">
            <h1 className="text-2xl font-semibold">
              Percorso esercizi - Cattura 2
            </h1>
            <p className="text-sm text-muted-foreground">
              Cattura le pietre marcate scegliendo la mossa corretta.
            </p>
          </div>
          ...
          {!completed ? (
            <div className="space-y-6">
              <div className="space-y-4 rounded-2xl border border-dashed border-[#d4f0b2] bg-[#f7fdf2] p-6">
                <header className="space-y-2 text-center">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {prompt || 'Cattura le pietre marcate.'}
                  </p>
                ...
                <div className="text-center">
                  {status === 'done' ? (
                    ...
                    <p className="text-xs text-muted-foreground">
                      Seleziona la mossa che cattura il gruppo per avanzare.
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className="space-y-4 text-center">
              <div className="inline-flex items-center gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-4 text-emerald-700 shadow-sm shadow-emerald-100">
                ...
                <span>Hai completato tutti gli esercizi di cattura 2!</span>
              </div>
              <button
