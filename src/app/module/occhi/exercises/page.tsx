...
const exercises = Array.from({ length: 10 }, (_, idx) => ({
  ...
  sgfPath: `/sgf/01_01_05_${String(idx + 1).padStart(2, '0')}.sgf`,
}));
...
export default function OcchiExercisesPage() {
  ...
  useEffect(() => {
    ...
    fetch(currentExercise.sgfPath)
      ...
      .then((text) => {
        ...
        setPrompt(
          root.comment?.trim() || 'Crea due occhi con la mossa corretta.',
        );
      ...
      .catch((err) => {
        if (!active) return;
        console.error(
          `[Occhi] Impossibile caricare ${currentExercise.sgfPath}`,
          err,
        );
  ...
  const statusMessage = useMemo(() => {
    ...
    if (status === 'done') return 'Corretto! La catena ha due occhi.';
    if (status === 'progress')
      return 'Continua la sequenza per creare due occhi.';
    if (status === 'error') return 'Non e la mossa giusta. Riprova.';
    return 'Seleziona la mossa che crea i due occhi.';
        ...
        <section className="rounded-3xl border border-emerald-100 bg-white/90 p-6 shadow-lg shadow-emerald-100 backdrop-blur">
          <div className="mb-6">
            <h1 className="text-2xl font-semibold">
              Percorso esercizi - Occhi
            </h1>
            <p className="text-sm text-muted-foreground">
              Crea due occhi per rendere viva la catena.
            </p>
          </div>
          ...
          {!completed ? (
                ...
                <div className="text-center">
                  {status === 'done' ? (
                    ...
                    <p className="text-xs text-muted-foreground">
                      Seleziona la mossa che crea i due occhi per avanzare.
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className="space-y-4 text-center">
              <div className="inline-flex items-center gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-4 text-emerald-700 shadow-sm shadow-emerald-100">
                ...
                <span>Hai completato tutti gli esercizi sugli occhi!</span>
              </div>
              <Link
                href="/dashboard"
